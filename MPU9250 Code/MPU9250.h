...
class MPU9250 {
    ...
    int lastUpdate;          // used to calculate integration interval
    int firstUpdate;         // used to calculate integration interval
    ...
    MPU9250(I2C &i2c_port, uint8_t address, uint8_t board):i2c(&i2c_port){
    ...
    lastUpdate = 0, firstUpdate = 0, Now = 0;  // used to calculate integration interval
  ...
  void led2_thread(void const *args) {
    ...
    void Calculations(){
        ...
        magbias[0] = 89.867859;   // User environmental x-axis correction in milliGauss, should be automatically calculated
        magbias[1] = 55.780052;  // User environmental x-axis correction in milliGauss
        magbias[2] = -177.798920;  // User environmental x-axis correction in milliGauss
       ...
       // if (delt_t > 500) { // update LCD once per half-second independent of read rate
        if (delt_t > 5) { // update LCD once per 0.005s independent of read rate
...
//           switch (boardNo) {
//  case 1:
//    pc.printf("Board 1:  q0 = %f   q1 = %f   q2 = %f   q3 = %f \n\r", q[0], q[1], q[2], q[3]);
//    break;
//  case 2:
//    pc.printf("Board 2:  q0 = %f   q1 = %f   q2 = %f   q3 = %f \n\r ", q[0], q[1], q[2], q[3]);
//    break;
//      case 3:
//    pc.printf("Board 3:  q0 = %f   q1 = %f   q2 = %f   q3 = %f \n\r ", q[0], q[1], q[2], q[3]);
//    break;
//      case 4:
//    pc.printf("Board 4:  q0 = %f   q1 = %f   q2 = %f   q3 = %f \n\r ", q[0], q[1], q[2], q[3]);
//    break;
//  default:
//   pc.printf("unknown \n");
//  }
            ...
            yaw   = atan2(2.0f * (q[1] * q[2] + q[0] * q[3]), 1 - 2.0f * (q[2] * q[2] + q[3] * q[3]));
            pitch = -asin(2.0f * (q[1] * q[3] - q[0] * q[2]));
            roll  = atan2(2.0f * (q[0] * q[1] + q[2] * q[3]), q[0] * q[0] - q[1] * q[1] - q[2] * q[2] + q[3] * q[3]);
            pitch *= 180.0f / PI;
            yaw   *= 180.0f / PI;
            //yaw   -= 13.8f; // Declination at Danville, California is 13 degrees 48 minutes and 47 seconds on 2014-04-04
            //yaw   -= -0.38f;  // Declination in London, UK
            roll  *= 180.0f / PI;
        ...
        switch (boardNo) {
 case 1:
    pc.printf("Board 1:  roll = %f   pitch = %f   yaw = %f   \n\r", roll, pitch, yaw);
    //pc.printf("Yaw, Pitch, Roll: %f %f %f\n\r", yaw, pitch, roll);
    break;
 case 2:
    pc.printf("Board 2:  roll = %f   pitch = %f   yaw = %f   \n\r", roll, pitch, yaw);
    break;
   case 3:
    pc.printf("Board 3:  roll = %f   pitch = %f   yaw = %f   \n\r", roll, pitch, yaw);
   break;
     case 4:
   pc.printf("Board 4:  roll = %f   pitch = %f   yaw = %f   \n\r", roll, pitch, yaw);
   break;
default:
   pc.printf("unknown \n");
}
 ...
 int16_t mag_max[3] = {0x8000, 0x8000, 0x8000}, mag_min[3] = {0x7FFF, 0x7FFF, 0x7FFF}, mag_temp[3];
 ...
 wait(0.1);  // at 8 Hz ODR, new mag data is available every 125 ms
 }
 //pc.printf("out of two loops");
 // Get hard iron correction
 mag_bias[0]  = (mag_max[0] + mag_min[0])/2;  // get average x mag bias in counts
 mag_bias[1]  = (mag_max[1] + mag_min[1])/2;  // get average y mag bias in counts
 mag_bias[2]  = (mag_max[2] + mag_min[2])/2;  // get average z mag bias in counts

pc.printf("mx_bias = %i \r \n",mag_bias[0]);
pc.printf("my_bias = %i \r \n",mag_bias[1]);
pc.printf("mz_bias = %i \r \n",mag_bias[2]);
getMres(); // Get magnetometer sensitivity

pc.printf("mRes = %f \r \n",mRes );
pc.printf("mag_bias_after_X = %f \r \n", ((float) 58)*mRes*magCalibration[0]);
pc.printf("mag_bias_after_Y = %f \r \n", ((float) 36)*mRes*magCalibration[1]);
pc.printf("mag_bias_after_Z = %f \r \n", ((float) -119)*mRes*magCalibration[2]);
